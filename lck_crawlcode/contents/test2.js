    ...
    data.each((idx,list)=>{
        if(idx<2){
            todayMatch[idx]={
                ...
                team2Name:"https://escorenews.com"+$(list).find("div.teams > span:nth-child(3) > picture > img").attr('src'),
                team2Logo:$(list).find("div.teams > span:nth-child(3) > b").text()//.replace($(list).find("b").html(),"")
            }
        }else{return false}
    });   
    return todayMatch
